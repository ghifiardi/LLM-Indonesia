...
export function buildCapabilityMapSpec(text, slideCount = 6) {
  if (!looksLikeCyberCapabilityMap(text)) return null;
  ...
  const slides = [
    ...
    ...parsed.domains.map((domain) => ({
      type: "cards",
      headline: `Detail Domain: ${domain.name}`,
      cards: domain.items.slice(0, 8).map((item) => ({
        title: item.name,
        desc: item.status
      }))
    })),
  ...
  const target = Math.min(30, Math.max(4, Number(slideCount) || 6));
  return { title, subtitle, slides: slides.slice(0, target - 1).concat(slides[slides.length - 1]) };
}

function looksLikeCyberCapabilityMap(text) {
  const value = String(text || "").toLowerCase();
  const hasCapabilityMapSignal = /\b(capability map|capability baseline|platform baseline|implementation status|implemented|not implemented|partially implemented)\b/i.test(value);
  const hasStrongCyberSignal = /\b(cyber|cybersecurity|security|keamanan|soc|siem|phishing|fraud|scam|threat|vulnerability|incident|zero trust|firewall|endpoint protection|edr|xdr|mfa|privileged access|access management)\b/i.test(value);
  const weakSignals = [
    /\bidentity\b/i,
    /\bdevices?\b/i,
    /\bnetwork\b/i,
  ...
  return hasCapabilityMapSignal && (hasStrongCyberSignal || weakSignals >= 5);
}
