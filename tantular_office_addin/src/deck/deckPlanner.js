...
Jumlah slide target: ${slideCount}. Jika target besar, gunakan ruang itu untuk cakupan yang lebih kaya: lebih banyak subtopik, section, contoh, metodologi, risiko, rekomendasi, dan next step. Jangan memadatkan semua konten ke sedikit slide.
...
export async function planDeck({ brief, slideCount = 6, tone = "", instruction = "" }) {
  ...
  try {
    raw = await runTantular({
      ...
      maxTokens: tokenBudgetForSlideCount(count),
      temperature: 0.25
    });
  } catch (error) {
