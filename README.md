...
- [Direct Ollama Indonesian SLM benchmark](#direct-ollama-indonesian-slm-benchmark)
- [Baseline / reference outputs](#baseline--reference-outputs)
- [Public / holdout evaluation](#public--holdout-evaluation)
- [Recipe optimization](#recipe-optimization)
- [DGM-lite recipe evolution](#dgm-lite-recipe-evolution)
...
# Directly benchmark your local Ollama SLM on Indonesian prompts
python3 -m godel_agent_prototype.benchmark_ollama_indonesian \
  --model qwen2.5-coder:1.5b \
  --split public \
  --out build/ollama_indonesian_public.json

# Optimize a prompt/recipe only after the benchmark exists
python3 -m godel_agent_prototype.recipe_optimizer \
  --model qwen2.5-coder:1.5b \
  --out build/ollama_recipe_optimization.json

# DGM-lite: archive-based open-ended recipe evolution (stepping stones)
python3 -m godel_agent_prototype.dgm_recipe_optimizer \
  --model qwen2.5-coder:1.5b \
  --iterations 16 \
  --archive-out build/dgm_archive.jsonl \
  --out build/dgm_report.json
...
- reference answers and generated baseline outputs are available;
- the stronger rubric reports per-dimension scores;
- deterministic holdout splits keep private cases out of feedback;
- direct benchmark and recipe optimizer dry-run paths work;
...
The `eval_sets/` directory holds 24 JSONL cases covering:
...
```json
{
  ...
  "weight": 1.2,
  "reference_answer": "Baik Kak, segera blokir kartu melalui ... kanal resmi ..."
}
```
...
| `reference_answer` | no | Hand-written reference output used for overlap diagnostics and reports |
| `baseline_outputs` | no | Optional stored baseline outputs; standard generic/keyword baselines can also be generated |

Scoring is still dependency-free, but now multi-dimensional. Each case reports `dimensions` for `term_coverage`, `safety`, `official_channel`, `actionability`, `tone_concision`, and `reference_overlap`, plus aggregate `dimension_means`. For serious work, extend this with exact checks where possible, retrieval-grounded factuality, safety classifiers, human labels, and held-out private tests.
...
## Direct Ollama Indonesian SLM benchmark

The advancement sequence starts by benchmarking the local SLM directly, before any self-improvement or recipe optimization:

```bash
python3 -m godel_agent_prototype.benchmark_ollama_indonesian \
  --model qwen2.5-coder:1.5b \
  --split public \
  --out build/ollama_qwen2_5_coder_1_5b_indonesian_public.json

python3 -m godel_agent_prototype.benchmark_ollama_indonesian \
  --model qwen2.5-coder:1.5b \
  --split holdout \
  --out build/ollama_qwen2_5_coder_1_5b_indonesian_holdout.json
```

The report includes model answers, reference answers, generated baseline outputs, per-case missing/forbidden terms, category means, and per-dimension rubric means. On the local `qwen2.5-coder:1.5b` smoke run, the direct public benchmark scored `0.507` across 17 public cases and the holdout audit scored `0.374` across 7 private cases.

Use `--limit N` for quick checks, and `--dry-run` when Ollama is unavailable.

## Baseline / reference outputs

Every eval case can carry a `reference_answer`. The `baselines.py` module also exposes two standard comparison policies:

- `generic_baseline_policy` — polite but intentionally weak.
- `keyword_baseline_policy` — simple non-learned routing for banking, fraud, Dukcapil, and DJP.

Benchmark JSON reports include `reference_answer` and `baseline_outputs` so model or recipe changes can be compared against stable non-optimized references.

## Recipe optimization

Only after direct benchmarking, references, rubric dimensions, and public/holdout splits are in place should recipe optimization run:

```bash
python3 -m godel_agent_prototype.recipe_optimizer \
  --model qwen2.5-coder:1.5b \
  --out build/ollama_qwen2_5_coder_1_5b_recipe_optimization.json
```

The optimizer evaluates candidate prompt recipes on public cases only, selects the best public recipe, and then audits it on holdout cases. On the local `qwen2.5-coder:1.5b` smoke run, the best recipe was `safety_first_official`, improving public score from `0.507` to `0.726` and holdout score from `0.374` to `0.674`.

## DGM-lite recipe evolution

The greedy `recipe_optimizer` only keeps the single best recipe. Inspired by the [Darwin Gödel Machine](https://arxiv.org/abs/2505.22954) (ICLR 2026), `dgm_recipe_optimizer` instead maintains an **archive** of every valid recipe variant and evolves them open-endedly:

```bash
python3 -m godel_agent_prototype.dgm_recipe_optimizer \
  --model qwen2.5-coder:1.5b \
  --iterations 16 \
  --archive-out build/dgm_qwen2_5_coder_1_5b_archive.jsonl \
  --out build/dgm_qwen2_5_coder_1_5b_report.json
```

How it mirrors the DGM paper, at recipe scale rather than full coding-agent scale:

- **Archive of stepping stones.** Every valid variant is kept (`recipe_archive.py`), not just the current best. A variant that looks mediocre now can be the parent of a later winner.
- **Open-ended parent selection.** Parents are chosen roughly in proportion to public score and inversely to how many children they already have, and every variant keeps a non-zero selection probability. Selection is deterministic given `--seed` so runs reproduce.
- **Single-dimension mutation.** Each child differs from its parent in exactly one reviewable dimension (`recipe_mutator.py`): safety directive, extra guidance, answer style, system-prompt lead, or few-shot exemplars.
- **Empirical validation.** Every child is scored on the **public** split with the multi-dimensional rubric before entering the archive.

Anti-Goodhart safeguards (from the paper's safety discussion):

- optimization uses **public cases only**; the holdout split is a strict **post-hoc audit**, never a target;
- candidate recipes are **validated** before they can become parents — a variant that drops its safety/official-channel posture is marked invalid and discarded, so it cannot seed further evolution.

On the local `qwen2.5-coder:1.5b` smoke run (16 iterations, 17 public / 7 holdout cases), the archive grew to 20 nodes and discovered a best recipe at **public 0.778 / holdout 0.679**. Crucially, that winner descended from the `domain_router` seed (0.717) rather than the best seed (`safety_first_official`, 0.726) — a stepping-stone lineage a greedy best-only search would never have explored. The archive JSONL records every node's `parent_id`, score, origin mutation, and validity for full lineage review.
...
| `EvalCase` | One support case (`query`, `required_terms`, `forbidden_terms`, `weight`, `category`, `reference_answer`). |
| `load_cases_from_dir(dir)` | Load all `*.jsonl` cases from a directory. |
| `score_answer(case, answer)` | Score one model/policy answer with the multi-dimensional Indonesian rubric. |
...
### Baselines and benchmark helpers

| Symbol | Module | Summary |
| --- | --- | --- |
| `generic_baseline_policy` | `baselines.py` | Weak generic Indonesian support baseline. |
| `keyword_baseline_policy` | `baselines.py` | Deterministic keyword-routing baseline. |
| `baseline_outputs_for_case(case)` | `baselines.py` | Generated comparison outputs included in benchmark reports. |
| `reference_output_for_case(case)` | `baselines.py` | Stored reference answer with fallback reference policy. |
| `benchmark_ollama_indonesian` | module | CLI for direct local Ollama benchmark before optimization. |
| `recipe_optimizer` | module | CLI for public-split recipe optimization and holdout audit. |
| `RecipeArchive` / `RecipeNode` | `recipe_archive.py` | DGM-style archive with lineage and open-ended parent selection. |
| `mutate_recipe(parent, seed, iteration)` | `recipe_mutator.py` | Deterministic single-dimension recipe mutation. |
| `evolve_recipes(...)` | `dgm_recipe_optimizer.py` | Archive-based open-ended recipe evolution + holdout audit. |
| `validate_recipe(recipe)` | `dgm_recipe_optimizer.py` | Reject variants that drop the safety/official-channel contract. |
| `dgm_recipe_optimizer` | module | CLI for DGM-lite recipe evolution. |
...
├── baselines.py                # generic/keyword baseline and reference helpers
├── benchmark_ollama_indonesian.py # direct local Ollama Indonesian benchmark
├── recipe_optimizer.py         # public-split recipe search + holdout audit
├── recipe_archive.py           # DGM-style recipe archive + parent selection
├── recipe_mutator.py           # deterministic single-dimension recipe mutations
├── dgm_recipe_optimizer.py     # DGM-lite open-ended recipe evolution + audit
