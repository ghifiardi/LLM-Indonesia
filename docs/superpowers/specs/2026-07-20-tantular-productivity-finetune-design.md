...
## Addendum 2 — 2026-08-09 — Serving runtime: llama-server (user decision, spike-evidenced)

Spike steps 5b/5c/5d produced a complete serving matrix for the merged sentinel model: merge and GGUF conversion are correct (transformers and `llama-cli` both reproduce the sentinel exactly), but **Ollama 0.24.0's engine mis-serves imported qwen3 GGUFs** (degenerate/non-deterministic output even at temperature 0; template overrides do not fix it), while **`llama-server` serves the identical Q4_K_M file correctly and deterministically** through the add-in's request shape (v9430, `reasoning_effort` field accepted, ~24-28 tok/s). Evidence: `tantular/finetune/spike/report.json` steps 5b/5c/5d; commits 7ef341b, b300912, 80f1c06.

**Decision (user, 2026-08-09):** the fine-tuned model ships served by **llama.cpp's `llama-server`**, managed by the Tantular Companion (base models remain on Ollama; revisit Ollama serving when upstream fixes imported-GGUF handling — tracked as a follow-up). Integration notes for the export task: the companion must manage the llama-server process and route the fine-tuned model's requests to it; qwen3 thinking must be disabled at the llama-server layer (flag/template), since `reasoning_effort` is accepted but not honored there. The sentinel model's content-channel bleed on unrelated prompts is an expected artifact of deliberate sentinel over-training and is NOT a toolchain defect; the real model's gates (prose A/B, format filters, frozen challenge set) guard against real degeneration.

**Gate status: PASSED** — sentinel behavior reproduces at every hop of the shipping chain (Tinker → PEFT → merge → GGUF → llama-server). Generation spend is unlocked; the pilot's stop-and-report budget rule remains in force.
